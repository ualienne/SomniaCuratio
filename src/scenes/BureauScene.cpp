...
BureauScene::BureauScene(sf::RenderWindow& window,
  ...
  if (m_font.openFromFile(m_assets.fontPath)) {
    m_fontLoaded = true;
    m_helpText = std::make_unique<sf::Text>(m_font, m_assets.helpText,
  ...
  if (m_map.loadFromFile(m_assets.mapPath)) {
    ...
    std::cerr << "[BureauScene] FAILED to load map\n";
  }
  ...
  const std::vector<NpcDef> npcDefs = {
      {"npc_voss", m_assets.voss.texture, m_assets.voss.name,
       m_assets.voss.greeting},
      {"npc_kira", m_assets.kira.texture, m_assets.kira.name,
       m_assets.kira.greeting},
      {"npc_hayes", m_assets.hayes.texture, m_assets.hayes.name,
       m_assets.hayes.greeting},
  };
  ...
  if (!m_player.loadTexture(m_assets.playerTexture))
    std::cerr << "[BureauScene] agent texture not loaded\n";
...
void BureauScene::onResume() {
  if (!m_pendingMessage.empty()) {
    DialogueManager::instance().showSingleReplica(m_assets.sysTitle,
                                                  m_pendingMessage);
